[Home](../../) / [Examples](index.md) / [ADVENTURE/3000 Port](adventure3000.md) / Stage 16: Real Scratch-Variable Names

<div class="prose" markdown="1">

# Stage 16: Real Scratch-Variable Names

A handful of cryptic scratch names slipped past stage 14's renaming pass
and stage 15's index-typing pass -- caught and fixed here.

Converted: `rc` (the main loop's dispatch-result holder, following
`performMove%()`'s own "0 = message printed, no move; 1 = moved, check
pits next" convention) -> `moved%`; `b1$` (`printMessage()`'s message-
fragment scratch string) -> `messageText$`; the top-level init section's
two `ii%` loop counters (already `%`-typed by stage 15, but never given
real names) -> `roomIndex%` (zeroing `itemRoom`/`roomVisited` for rooms
1-99) and `itemPointsIndex%` (reading `itemPoints` in groups of 5); and
the `dx1`..`dx9`/`dx0` scratch fields used three times over to read
fixed-width records (a 10-wide `roomExits` row, then twice more for
5-wide `itemRoom`/`itemPoints` rows) -> a single deliberately generic
`field1`..`field10` family, the same treatment stage 14 gave `z0` (kept
a generic name on purpose, since it's genuinely reused for unrelated
record shapes, not given one bespoke name per call site).

Left alone, deliberately: `i%` (`parseAndDispatchCommand%()`'s
keyword-flag reset loop) -- already named and typed by stage 15, and
"i" for a plain reset loop isn't the same kind of cryptic as
"dx"/"ii"/"rc"/"b1$".
...
<summary><code>stage16-refactored-bascal/adventure.bcl</code> -- Stage 16: Real Scratch-Variable Names</summary>

```bascal

// ADVENTURE/3000 -- Stage 16: the last few cryptic scratch names that
// slipped past stage 14's renaming pass (and stage 15's index-typing
// pass) get real names too. Started as an exact copy of stage 15; see
// ../README.md for the port's provenance and staging.
//
// Converted: `rc` (the main loop's dispatch-result holder, following
// performMove%()'s own "0 = message printed, no move; 1 = moved, check
// pits next" convention) -> `moved%`; `b1$` (printMessage()'s message-
// fragment scratch string) -> `messageText$`; the top-level init
// section's two `ii%` loop counters (already `%`-typed by stage 15,
// but never given real names) -> `roomIndex%` (zeroing itemRoom/
// roomVisited for rooms 1-99) and `itemPointsIndex%` (reading
// itemPoints in groups of 5); and the `dx1`..`dx9`/`dx0` scratch fields
// used three times over to read fixed-width records (a 10-wide
// roomExits row, then twice more for 5-wide itemRoom/itemPoints rows)
// -> a single deliberately generic `field1`..`field10` family, the
// same treatment stage 14 gave `z0` (kept a generic name on purpose,
// since it's genuinely reused for unrelated record shapes, not given
// one bespoke name per call site).
//
// Left alone, deliberately: `i%` (parseAndDispatchCommand%()'s
// keyword-flag reset loop) -- already named and typed by stage 15,
// and "i" for a plain reset loop isn't the same kind of cryptic as
// "dx"/"ii"/"rc"/"b1$".
//
// Verified with `bcc --check`; `--target basic` still compiles with
// zero warnings; and a `--target c` smoke test (the same command
// sequence as every prior stage, plus a SAVE GAME round trip) byte-
// identical against stage 15.
...
[← Stage 15: Integer-Typed Array Indices](adventure3000-stage15.md) [Next: Development Journey →](../journey.md)
