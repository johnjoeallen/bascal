[Home](../index.md) / Why BASCAL exists
...
Both panes run on the same classic BASIC. `bcc` transpiles the pane on the right straight down to the pane on the left's shape — you just never have to number the branches or wire the `GOTO`s by hand. See the [structured control-flow chapter](control-flow.md) for loops and short-circuit conditions.
