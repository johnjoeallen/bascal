...
Every array-parameter example across the manual, the GitHub Pages mirror, and the tutorial sort and statistics libraries got migrated to the new convention in the same pass — 18 files, +642/−630 lines. Two of the GitHub Pages tutorial snippets were missed in that sweep — `08_arrays.html` and `12_require.html` still showed the old, manually-counted calling style — and got caught and fixed once specifically asked whether a `sizeof()` tutorial existed at all. The reversal is documented in the commit's own words, worth quoting rather than paraphrasing:
...
Regenerating the affected tutorial outputs confirmed the fix concretely: `tutorial/08_arrays.bas`, where `printArray%` is called twice, now emits `DIM printarray_arr_0%(6)` exactly once, at the top of the file, instead of twice at each call site.
