...
`bcc` also ships its own source formatter, in the same spirit as `gofmt`: `--format-check` reports every line whose indentation, spacing, or casing doesn't match `bcc`'s own canonical style (without touching the file, exiting non-zero if it finds any), and `--format` rewrites the file in place. It never reflows an expression or changes how many lines a file has — every fix is pure whitespace, a `wend`/`loop` → `end while`/`end do` rewrite, lowercasing a reserved word or a builtin like `LEN`/`RND` (`IF` → `if`, `RND` → `rnd`), or rewriting a variable/function reference to match how its own declaration was written (`MYCOUNT%` → `myCount%`, for a `dim myCount%` elsewhere in the file -- or in a `require`d library, resolved the same way `bcc` itself resolves one) — so it's safe to run on a file mid-edit, or across an entire ported-from-BASIC case study in one pass. See the [Command-Line Reference](../manual/command-line-reference.md#source-formatting) for the full rule set.
