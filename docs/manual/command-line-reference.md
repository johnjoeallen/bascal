...
Tutorials 01 through 07, 11 (screen I/O subset), 18 (standard-library functions), and 20 (scalar methods) currently compile, assemble, and run on the JVM; tutorials using arrays, file/record I/O, shared COMMON state, or structured error handling remain marked Basic/C until their required features are implemented.
