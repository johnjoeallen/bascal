...
| `--target <t>`          | `-t`  | Backend to generate code for: `basic` (the original, complete backend), `C` (a native-C backend — see below), or `jvm` (a brand-new, bootstrap-stage native-JVM backend, still coming together — see below). Case-insensitive; `c`/`jvm` also work.                                                                                                                                                                                                                                                                 |
...
`--target jvm` is coming: a brand-new, bootstrap-stage native-JVM backend, not yet ready for real programs. Currently only `print` of string literals and `end` are supported; see the GitHub issue tracker's `jvm-target` label for progress.
