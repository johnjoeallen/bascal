...
Tutorials 01 through 08, 11 (screen I/O subset), 18 (standard-library functions), and 20 (scalar methods) currently transpile on the JVM; tutorials 01 through 07, 11, 18, and 20 also compile, assemble, and run there. Tutorial 08 exercises the current integer-array implementation but remains pending runtime validation until array parameter semantics (#112), non-integer arrays (#111), and genuine multidimensional semantics (#113) are complete. Tutorials using file/record I/O, shared COMMON state, or structured error handling remain marked Basic/C until their required features are implemented.
