...
| `--check`               |       | Parse the source and every required library without resolving it or generating backend output. This is useful for planned language syntax whose typed IR or backend implementation is not yet available. It cannot be combined with `--binary` or `--run`.                                                                                                                                                                                  |
| `--binary`              | `-b`  | Compile the generated output to a binary in `tmp/`: `fbc` for `--target basic`, `gcc` for `--target c`, or (for `--target jvm`) assembles to a `.class` using the assembler linked directly into `bcc` -- see below.                                                                                                                                                                                                                                                                                                                         |
...
- Classic `ON ERROR GOTO`/every `RESUME` variant is rejected, for the same reason as `--target c` above. The JVM target supports structured `try`/`catch`/`finally` for explicit numeric `throw`/`error` statements, including error-code filters and source-file bindings. Runtime file-operation errors are tracked separately in issue #115. **Portable equivalent:** use structured `try`/`catch`/`finally` rather than legacy handlers.
...
Tutorials that compile end to end today (see each one's `.c` counterpart for its output): [`hello`](https://github.com/johnjoeallen/bascal/blob/main/tutorial/hello.bcl), [`variables`](https://github.com/johnjoeallen/bascal/blob/main/tutorial/variables.bcl), [`arithmetic`](https://github.com/johnjoeallen/bascal/blob/main/tutorial/arithmetic.bcl), [`conditions`](https://github.com/johnjoeallen/bascal/blob/main/tutorial/conditions.bcl), [`loops`](https://github.com/johnjoeallen/bascal/blob/main/tutorial/loops.bcl), [`select_case`](https://github.com/johnjoeallen/bascal/blob/main/tutorial/select_case.bcl), [`functions`](https://github.com/johnjoeallen/bascal/blob/main/tutorial/functions.bcl), [`arrays`](https://github.com/johnjoeallen/bascal/blob/main/tutorial/arrays.bcl), [`data`](https://github.com/johnjoeallen/bascal/blob/main/tutorial/data.bcl), [`files`](https://github.com/johnjoeallen/bascal/blob/main/tutorial/files.bcl), [`screen`](https://github.com/johnjoeallen/bascal/blob/main/tutorial/screen.bcl), [`shared/start`](https://github.com/johnjoeallen/bascal/blob/main/tutorial/shared/start.bcl), [`shared/show`](https://github.com/johnjoeallen/bascal/blob/main/tutorial/shared/show.bcl), [`random_and_record_files`](https://github.com/johnjoeallen/bascal/blob/main/tutorial/random_and_record_files.bcl), [`short_circuit`](https://github.com/johnjoeallen/bascal/blob/main/tutorial/short_circuit.bcl), [`portable_error_handling`](https://github.com/johnjoeallen/bascal/blob/main/tutorial/portable_error_handling.bcl), [`restore_data`](https://github.com/johnjoeallen/bascal/blob/main/tutorial/restore_data.bcl), [`stdlib`](https://github.com/johnjoeallen/bascal/blob/main/tutorial/stdlib.bcl), and [`inventory`](https://github.com/johnjoeallen/bascal/blob/main/tutorial/inventory.bcl). The GitHub issue tracker's `c-target` label lists what's still missing to get the rest of the tutorials compiling.
...
- Numeric array parameters, `byval` or `byref`, declared with one `?` per dimension (`arr%(?)`, `grid%(?, ?)`). `byval` copies the caller's contiguous elements into a per-call local C99 variable-length buffer sized from the actual argument; `byref` reuses the caller's own storage directly, with no copy.
...
`--target jvm` is an early-stage native-JVM backend. It compiles BASCAL to
low-level Krakatau assembly, rather than transpiling it to another source
language; Krakatau's `krak2` assembler, linked directly into `bcc` as a
library (no separate install or `PATH` lookup needed), assembles that output
into a class file for a JRE. It currently supports scalar variables and
constants plus the `PI` constant,
numeric arithmetic and comparisons,
`ABS`/`SQR`/`INT`/`FIX`/`SGN`/`SIN`/`COS`/`TAN`/`ATN`/`LOG`/`EXP`/`RND`/`CINT`/`CLNG`/`CSNG`/`CDBL`/`VAL`/`INSTR`/`MIN`/`MAX`, string concatenation and selected string conversions including `TRIM$`/`SPACE$`/`STRING$`, `print`, `if`/`elseif`/`else`, `for`/`while`/`do` loops with `exit`, numeric or exact-string `select case`, by-value scalar functions and procedures with local variables, `global` access to top-level scalars, in-memory record values (complete-literal initialization, copying, and member reads/writes), and one-/multi-dimensional integer arrays. Integer function and procedure array parameters use native JVM references: `byref` passes the caller's array directly, while `byval` deep-copies ranks 1 through 8. Per-call array bounds/`SIZEOF`, non-integer arrays, file/screen I/O, and many standard-library operations remain unfinished; see the GitHub issue tracker's `jvm-target` label for progress.

Tutorials 01 through 08, 11 (screen I/O subset), 12 (require), 14 (procedures), 16 (short-circuit conditions), 18 (standard-library functions), 20 (scalar methods), and 21 (portable structured error handling) currently transpile on the JVM; all of those also compile and assemble there. Tutorial 08 validates the current integer-array declarations, indexed access, function array calls, and 2-D example; tutorial 14 validates procedure calls, including integer array parameters. Per-call array bounds (#126), non-integer arrays (#111), file/record I/O, shared COMMON state, classic BASIC error handling, and RESTORE/DATA remain outside the JVM-supported tutorial set.

It emits [Krakatau](https://github.com/Storyyeller/Krakatau) assembly text (`.j`). Return-only programs can be written as class files by the built-in writer; other output is assembled into a real `.class` by Krakatau's `v2`/Rust assembler (`krakatau2`), pulled in as a Rust library dependency and linked directly into `bcc` -- no separate install, `PATH` lookup, or config setting needed -- then run by any JRE's `java`. (BASCAL depends on [a small fork](https://github.com/johnjoeallen/Krakatau) that exposes Krakatau's assembler as a reusable library crate, pending [an upstream PR](https://github.com/Storyyeller/Krakatau/pull/216) to add that support to Krakatau itself.) Many thanks to Storyyeller and the [Krakatau](https://github.com/Storyyeller/Krakatau) project, without which `--target jvm` would have no assembler at all.
