...
language; `krak2` assembles that output into a class file for a JRE. It
currently supports scalar variables and constants plus the `PI` constant,
...
It emits [Krakatau](https://github.com/Storyyeller/Krakatau) assembly text (`.j`). Return-only programs can be written as class files by the built-in writer; other output is assembled into a real `.class` by the external `krak2` tool (`v2`/Rust branch — no installed-by-default story, so `scripts/fetch-krak2.sh` builds and installs it into `~/.local`, matching how `scripts/install-bcc.sh` installs `bcc` itself), then run by any JRE's `java`.
