...
For the `c` target, the opposite is true: there's no BASIC runtime underneath at all. `if`/`for`/`while` compile to C's own native control flow, and BASCAL's C backend has its own, different runtime rather than reusing BASIC's, so not everything carries over. Statements whose behavior depends on BASIC's own runtime — `CHAIN` being the clearest example, since it depends on BASIC's program-swapping semantics — aren't supported under `--target c`. See the [Backends](command-line-reference.md#backends) section of the [Command-Line Reference](command-line-reference.md) for exactly what is and isn't supported today.

The `jvm` target is the same story again, but against the JVM's own method/bytecode model instead of a real C call stack — no BASIC runtime underneath, its own different runtime instead, and, being newer and much less far along than `c`, far less of BASCAL currently carries over. See the same [Backends](command-line-reference.md#backends) section for what it supports so far.

**BASCAL started as a strict superset of classic BASIC — `--target basic` still is one.** Raw statements from the target dialect — `OPEN`/`FIELD`/`GET`/`PUT` for random-access files, bitwise `AND`/`OR`/`NOT` — still pass through unchanged there. `GOTO`/`GOSUB`/`ON ERROR GOTO`/ `RESUME`/`RESTORE` are raw BASIC too, but with one restriction: BASCAL manages line numbering itself, so their targets must be a `name:` label declared in source, never a raw line number — see [Labels](miscellaneous-statements.md#labels). Beyond that, wherever this manual documents a BASCAL construct for something (`select case` instead of an `IF`/`GOTO` dispatch chain, `record`/`file` instead of hand-written `FIELD`/`GET`/`PUT`, `&&`/`||` instead of bitwise short-circuit workarounds), treat that construct as the canonical way to write it in `.bcl` source — the original BASIC syntax is what the transpiler exists to get you away from, not an equally good alternative.

Once there's more than one output runtime to answer to, that strict-superset promise can't hold everywhere: `--target c` and `--target jvm` each permanently drop a small set of raw-BASIC forms that don't translate safely onto their own runtime (see [Portability across backends](command-line-reference.md#portability-across-backends) for the itemized list and each one's structured, portable equivalent). BASCAL as a whole is therefore a **partial** superset of BASIC, not a strict one — `--target basic` is the exception that keeps the original promise in full, and remains the most compatible choice whenever that matters more than a native binary or JVM output.
