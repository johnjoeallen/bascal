...
I had also become very enamoured with Turbo Pascal — my first real exposure to Niklaus Wirth's language. Wirth was, and remains, one of the most prolific language designers in the field: Pascal, Modula-2, Oberon, and further back Algol W, each one a working argument for discipline and clarity over cleverness. That sensibility is what made Pascal itself so appealing, and the Turbo Pascal environment matched it: an integrated editor, compiler, and run loop that made development feel lightning fast. Nothing in Ramtech BASIC brought that same fast edit-compile cycle to classic BASIC, but that experience shaped what I wanted: more structure, less friction, and a development process that felt more flexible.
...
The first version was a weekend project, written in Pascal itself — fitting, given how much of its structural thinking traced straight back to Wirth — and it supported only two things: `{label}` and `@include`. I demoed it to my boss, and he liked the idea enough to give me a week to rewrite it in BASIC so it could live inside the same environment as the programs it was helping to build.
...
It is not intended to be exactly what I built in the 1980s. It is closer to what I wish I could have built then — the tool I wish I'd had the skills and tools for back in 1985. Building a real transpiler, rather than another preprocessor, was finally within reach with Claude and Codex doing the heavy lifting. The result is BASCAL: not a Microsoft BASIC compiler, but a structured language that `bcc` transpiles into classic Microsoft BASIC, keeping the original global BASIC model while adding a more disciplined source language and a more practical build workflow. The language itself is significantly more advanced than the original: more structured, easier to read, easier to write, and organized into reusable files.

The BASCAL compiler (`bcc`) transpiles `.bcl` source files into generated `.bas` output by default. It supports multiline `if` / `else` / `end if`, `for` / `next`, `while` / `wend`, `function` declarations with explicit `return`, BASIC type suffixes, comments preserved in generated output, and path-style `require` / `import` dependencies.

BASCAL started as a strict superset of that classic BASIC too: bitwise `AND`/`OR`/`NOT` and hand-written `OPEN`/`FIELD`/`GET`/`PUT` still pass through unchanged. `GOTO`/`GOSUB` are raw BASIC too, but BASCAL manages line numbering itself, so their targets are always a `name:` label declared in source, never a raw line number. Beyond that, wherever BASCAL has its own construct for something, that construct is the canonical way to write it in `.bcl` source — the original BASIC syntax is what you're transpiling *away from*, not an equally-good alternative.

That strict-superset promise held as long as `bcc` had one output language to answer to. It no longer does: `--target C` and `--target jvm` transpile the same `.bcl` source onto a real C call stack and the JVM's own method model instead of reusing BASIC's own runtime, and each has to give up a small, permanent set of raw-BASIC forms that don't survive that translation safely (classic `ON ERROR GOTO`/`RESUME` on both; `GOSUB` and any `GOTO` crossing a function/procedure boundary on `--target jvm` specifically). BASCAL as a whole is now a **partial** superset of BASIC, not a strict one — `--target basic` is the one that still keeps the original promise in full, and remains the most compatible choice whenever that matters more than a native binary or JVM output. See [Portability across backends](manual/command-line-reference.md#portability-across-backends) for the itemized list and each divergence's structured equivalent.
...
BASCAL deliberately preserves BASIC's global runtime model — functions transpile to `GOSUB` and use generated global parameter/result storage — while adding source-level scope that keeps larger programs maintainable. Variables used inside a function or procedure are local unless that body explicitly declares them with `global` to access a top-level variable. Path-style names are dependency selectors, not runtime namespaces. Every parameter is copied in before the call; `byref` copies the result back out afterward too (`byval`, the default, doesn't). Array parameters support any number of dimensions, matched against how the callee's body indexes them — a mismatch is a transpile-time error. Recursive functions are not supported, because a recursive call would overwrite its own global parameter state.
...
[← Back to Why BASCAL](./#why)  ·  [Read the development journey →](journey.md)
